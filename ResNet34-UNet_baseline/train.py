...
print(f"Model   : ResNet34-UNet  |  Classes: {NUM_CLASSES}")  # 打印新模型名称
...
# 对 IoU 数组和像素统计数组进行切片 [:-1]，丢弃最后一个索引(11, Void类)
    ...
    # 【关键切换 4】保存文件重命名为 best_unet_cbam.pth
    if current_miou > best_miou:
        best_miou = current_miou
        torch.save(model.state_dict(), "unet.pth")
        print(f"         -> best mIoU updated: {best_miou*100:.2f}%  [saved unet.pth]")
...
print(f"\n✅ 阶段一训练彻底完成！ResNet34-UNet全局最佳 mIoU 为: {best_miou*100:.2f}%")